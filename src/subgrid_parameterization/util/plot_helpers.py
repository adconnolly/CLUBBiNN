...
def xp2_xpyp_term_dp1_lhs_quickPlots(
    model, dataset, y_text=["U2DFSN", "V2DFSN"], device=device("cpu")
):
    l1 = "Predicted"
    l2 = "True"
    ...
    for ivar in range(nvar):
        for k in range(nz):
            ...
            r[ivar, k] = np.corrcoef(
                y_test[:, ivar * nz + k], y_pred[:, ivar * nz + k]
            )[0, 1]
        ...
        ax2[ivar].scatter(
            y_test[:, ivar * nz : (ivar + 1) * nz],
            y_pred[:, ivar * nz : (ivar + 1) * nz],
        )
...
def UP_CBL_quickPlots(model, loader, y_text, device):
    ...
    for data in loader:
        ...
        for i in range(len(y_text)):
            r2 = np.mean(
                [
                    r2_score(y_test[:, i * (nz - 1) + k], y_pred[:, i * (nz - 1) + k])
                    for k in range(nz - 1)
                ]
            )
            r = np.mean(
                [
                    np.corrcoef(
                        y_test[:, i * (nz - 1) + k], y_pred[:, i * (nz - 1) + k]
                    )[0, 1]
                    for k in range(nz - 1)
                ]
            )
            ...
            ax2[i].scatter(
                y_test[:, i * nz : (i + 1) * nz], y_pred[:, i * nz : (i + 1) * nz]
            )
        ...
        for i in range(len(y_text)):
            for it in range(y_pred.shape[0]):
                ax[i].plot(
                    y_pred[it, i * (nz - 1) : (1 + i) * (nz - 1)], z, "r", label=l1
                )
                ax[i].plot(
                    y_test[it, i * (nz - 1) : (1 + i) * (nz - 1)], z, "k", label=l2
                )
                l1, l2 = "__nolegend__", "__nolegend__"
